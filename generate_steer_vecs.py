...
def get_args(run_in_notebook = False):
    ...
    parser.add_argument("--model_index", type=int, default=14, help="Index of the model to use, \
...
thinking_model = args.model_index in [5, 13, 14]  # Qwen3-8B, Qwen3-4B, Olmo-3-7B-Think

if thinking_model:
    if args.model_index in [5, 13, 14]: # Qwen3-8B, Qwen3-4B, Olmo-3-7B-Think
    ...
    for task in data:
        ...
        for d in task:
            ...
            if chat_enabled:
                ...
                if thinking_model and 'olmo' in model_name.lower():
...
for t in range(concept_iterator):
    for i, loc in enumerate(extraction_locs):
        for l, layer in enumerate(extraction_layers):
            ...
            plt.close()
            print(f"Saved {path}layer_{layer}_loc_{loc}_{args.target_tokens}_tsne.png")
            # plt.show()
        # break  
