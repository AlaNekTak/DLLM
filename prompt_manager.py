...
def get_prompt(domain_name, prompt_method, template_name, concept, prompt_strength, num_shots = 5):
    ...
    if domain_name == 'personalities':
        ...
        elif template_name == 'template_4':
            assert prompt_strength == 'low', "Template 4 only supports 'low' prompt strength"
            system_prompt_suffix = f"Pretend you are a human experiencing 'a little {concept_}'. Emotions can sometimes, and only to some extent, influence your decisions."
        elif template_name == 'template_5':
            assert prompt_strength == 'low', "Template 5 only supports 'low' prompt strength"
            system_prompt_suffix = f"Pretend you are a human experiencing 'a little {concept_}'. Emotions sometimes influence decisions to a limited degree."
        ...
        assert prompt_method == 'zero', "Humanoid domain only supports zero-shot prompts"
        assert prompt_strength == 'medium', "Humanoid domain only supports medium prompt strength"
        assert concept in ['human'], "Humanoid domain only supports 'human' concept"
        if template_name == 'template_1':
            system_prompt_suffix = f"Act as if you are a human."
        else:
            raise ValueError(f"Not implemented humanoid prompt template {template_name}")
    else:
...
if __name__ == '__main__':
    ...
    for prompt_method in ['zero', 'few', 'p2']:
        # for strength in ['low', 'high', 'medium', 'neg']:
        #     for template_name in ['template_1', 'template_2', 'template_3', 'template_4', 'template_5']:
        for strength in ['low', 'high', 'medium', 'neg']:
            for template_name in ['template_1', 'template_2', 'template_3', 'template_4', 'template_5']:
