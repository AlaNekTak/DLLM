...
class RiskCurveConfig:
    seed: int = 42
    p_list: Tuple[float, ...] = (0.05, 0.10, 0.20, 0.40, 0.60, 0.80, 0.90, 0.95)
    G_list: Tuple[int, ...] = (10, 20, 50, 100, 200, 500)
    s_multipliers: Tuple[float, ...] = (0.70, 0.85, 1.00, 1.15, 1.30)
    step_small: int = 1
    step_large: int = 5
    prob_estimate_per_pair: float = 0.1
...
class RiskCurveDataset:
    ...
    def _make_trials(self) -> List[Dict]:
        ...
        for p in self.cfg.p_list:
            ...
            for G in self.cfg.G_list:
                EV = p * G
                for q in self.cfg.s_multipliers:
                    S_raw = q * EV
                    step = self.cfg.step_large if S_raw >= 50 else self.cfg.step_small
                    S = max(step, int(round(S_raw / step) * step))
                ...
                if rng.random() < self.cfg.prob_estimate_per_pair:
    ...
    def get_unique_name(self): return "risk_curve"
    ...
    def finalize(self, save_path: Optional[str] = None):
        ...
        jsonl_path = os.path.join(out_dir, "risk_curve.jsonl")
        with open(jsonl_path, "w", encoding="utf-8") as f:
            for rec in self.records:
                f.write(json.dumps(rec) + "\n")
        print(f"[RiskCurve] Raw JSONL saved -> {jsonl_path}")
