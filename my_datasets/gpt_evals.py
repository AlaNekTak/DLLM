...
class GPTEmotionToneRegressor(GPTEvaluator):
    def __init__(self, test_mode: bool = False, offline: bool = False):
        super().__init__(test_mode, offline)
    
    def regress_emotion(self, response: str, expected_emotion: str = "joy"):
        ...
        messages = [
            {
                "role": "system",
                "content": (
                    "You are an expert evaluator of emotional tone in text. "
                    "You judge tone only, not factual content or intent."
                )
            },
            {
                "role": "user",
                "content": (
                    f"Evaluate the emotional tone of the text below.\n\n"
                    f"Target emotion: {expected_emotion}\n\n"
                    f"Rate how strongly this emotion is expressed in tone on an integer scale from 1 to 10, where:\n"
                    f"1 = not present at all\n"
                    f"10 = extremely strong and dominant\n\n"
                    f"Only return a single integer. Do not include explanations, labels, or punctuation.\n\n"
                    f"Text:\n"
                    f"\"\"\"\n{response}\n\"\"\"\n"
                )
            }
        ...
        try:
            score = int(re.search(r"\d+", raw_response.strip()).group())
        except Exception:
            score = -1.0
        
        return raw_response, score
    
    def process_batch(self, responses: list, expected_emotions: list[str], verbose: bool = False):
        raw_responses = []
        scores = []
        iterator = zip(responses, expected_emotions)
        if verbose:
            iterator = tqdm(iterator, total=len(responses), desc="Processing batch")
        for response, expected_emotion in iterator:
            raw_response, score = self.regress_emotion(response, expected_emotion)
            raw_responses.append(raw_response)
            scores.append(score)
        return raw_responses, scores
